      ...
      .then((campsite) => {
        if (campsite && campsite.comments.id(req.params.commentId)) {
          ...
          if (
            ...
            err = new Error(
              `You are not authorized to edit someone else's comment!`
            );
            err.status = 403;
            return next(err);
          }
      ...
      .then((campsite) => {
        if (campsite && campsite.comments.id(req.params.commentId)) {
          if (
            ...
            err = new Error(
              `Your user id is ${req.user._id} but the comment was written by ${
                campsite.comments.id(req.params.commentId).author._id
              }. You are not authorized to delete someone else's comment!`
            );
            err.status = 403;
            return next(err);
          }
